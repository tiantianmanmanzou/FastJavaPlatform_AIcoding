...
import { Table, type TableProps, type ColumnType } from '@ui'
...
const DataTable = React.forwardRef<any, DataTableProps>(({
  ...
  const convertColumns = (vueCols: TableColumn[]): ColumnType<any>[] => {
    const antdColumns: ColumnType<any>[] = []
    ...
    const processColumns = (cols: TableColumn[]): ColumnType<any>[] => {
      return cols.map((col) => {
        const antdCol: any = {
          title: col.label,
          dataIndex: col.prop,
